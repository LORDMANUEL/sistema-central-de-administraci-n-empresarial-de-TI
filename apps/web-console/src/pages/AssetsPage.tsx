...
export function AssetsPage() {
  ...
  return <section className="section section--flush"><div className="toolbar"><div><strong>Inventario canónico</strong><span className="muted"> {selectedTenant?.name ?? 'Sin empresa'}{selectedSite ? ` · ${selectedSite.name}` : ''}</span></div><button className="button button--primary" disabled={!scope.tenantId} onClick={()=>setShow(!show)}><Plus size={16}/>Nuevo activo</button></div>{show&&<form className="inline-form" onSubmit={submit}><label>Empresa<input value={selectedTenant?.name ?? ''} readOnly /></label><label>Sede<input value={selectedSite?.name ?? 'Sin sede específica'} readOnly /></label><label>Nombre<input required value={name} onChange={e=>setName(e.target.value)}/></label><label>Hostname<input value={hostname} onChange={e=>setHostname(e.target.value)}/></label><label>Tipo<input required value={type} onChange={e=>setType(e.target.value)}/></label><button className="button button--primary" disabled={create.isPending}>Guardar</button>{create.error&&<p className="form-error">{(create.error as Error).message}</p>}</form>}{assets.isLoading?<LoadingState/>:assets.error?<ErrorState message={(assets.error as Error).message}/>:scoped.length?<div className="table-wrap"><table><thead><tr><th>Activo</th><th>Hostname</th><th>Tipo</th><th>Estado</th><th>Sede</th></tr></thead><tbody>{scoped.map(a=><tr key={a.guardian_asset_id}><td><strong>{a.display_name}</strong><small>{a.guardian_asset_id.slice(0,8)}…</small></td><td>{a.hostname??'—'}</td><td>{a.asset_type}</td><td><StatusBadge value={a.status}/></td><td>{scope.sites.find(site=>site.id===a.site_id)?.name??(a.site_id?a.site_id.slice(0,8)+'…':'—')}</td></tr>)}</tbody></table></div>:<EmptyState title="Sin activos" body="No hay activos en el alcance seleccionado."/>}</section>
}
