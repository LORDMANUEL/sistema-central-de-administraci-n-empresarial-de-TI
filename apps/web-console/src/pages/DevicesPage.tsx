...
export function DevicesPage() {
  ...
  const devices = useQuery({
    queryKey: ['devices', tenantId, state],
    queryFn: () => api.get<Device[]>(`/devices${api.query({ tenant_id: tenantId, state })}`),
    enabled,
  })
  ...
  const loading = scopeLoading || (enabled && (devices.isLoading || assets.isLoading))
