...
export function CommandsPage() {
  const { tenantId, siteId, loading: scopeLoading } = useTenantScope()
  const [state, setState] = useState('')
  const queryClient = useQueryClient()
  const enabled = Boolean(tenantId)
  const commands = useQuery({
    queryKey: ['commands', 'global', tenantId, state],
    queryFn: () => api.get<Command[]>(`/commands${api.query({ tenant_id: tenantId, state, limit: 100 })}`),
    enabled,
    refetchInterval: 15_000,
  })
  const assets = useQuery({
    queryKey: ['assets', 'scope-support', tenantId],
    queryFn: () => api.get<Asset[]>(`/assets${api.query({ tenant_id: tenantId })}`),
    enabled,
  })
  ...
  const loading = scopeLoading || (enabled && (commands.isLoading || assets.isLoading))
