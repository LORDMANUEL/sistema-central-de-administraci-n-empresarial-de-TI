...
_ALLOWED_TENANT_AUDIT_ROLES = frozenset({"org_admin", "security_admin", "auditor"})
...
class IdentityAccessVerifier:
    ...
    def _load_jwks(self, *, force: bool = False) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            try:
                ...
                raise GuardianError(503, "audit.identity_unavailable", "Identity public keys are unavailable") from exc
            if not isinstance(payload, dict) or not isinstance(payload.get("keys"), list):
                raise GuardianError(503, "audit.identity_invalid_jwks", "Identity public key response is invalid")
            ...
            for item in self._load_jwks(force=force).get("keys", []):
                if item.get("kid") == kid:
                    try:
                        ...
                        raise GuardianError(401, "audit.invalid_token", "Identity access token is invalid") from exc
        raise GuardianError(401, "audit.invalid_token", "Identity access token is invalid")
    ...
    def verify(self, token: str) -> IdentityPrincipal:
        try:
            ...
            raise GuardianError(401, "audit.token_expired", "Identity access token has expired") from exc
...
def enforce_audit_read(
    ...
    if tenant_id is None or not tenant_id.strip():
        raise GuardianError(403, "audit.tenant_required", "Tenant scope is required")

    decision = resolver(tenant_id, principal.user_id, principal.bearer_token)
    if not decision.allowed:
        raise GuardianError(403, "audit.access_denied", "You do not have access to this tenant")
    if decision.tenant_status != "active":
        raise GuardianError(403, "audit.tenant_suspended", "Tenant is suspended")
    if decision.role not in _ALLOWED_TENANT_AUDIT_ROLES:
        raise GuardianError(403, "audit.role_not_allowed", "Your tenant role cannot read audit records")
    return decision
...
def current_principal(
    ...
    if credentials is None or credentials.scheme.lower() != "bearer":
        raise GuardianError(401, "audit.authentication_required", "Authentication is required")
