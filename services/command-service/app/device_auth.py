...
def current_device_principal(request:Request):
    ...
    if not settings.trusted_proxy_token or request.headers.get("X-Guardian-Proxy-Token","")!=settings.trusted_proxy_token:raise GuardianError(401,"device_auth_required","Trusted endpoint authentication required")
