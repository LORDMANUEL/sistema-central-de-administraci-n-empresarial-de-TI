...
def _database(tmp_path):
    engine = build_engine(f"sqlite+pysqlite:///{tmp_path / 'pki-outbox.db'}")
...
def test_event_envelope_matches_guardian_schema_v1(tmp_path):
    ...
    try:
        ...
        assert envelope["type"] == "pki.certificate.issued"
        assert envelope["aggregate_type"] == "certificate"
        assert envelope["aggregate_id"] == "cert-0"
...
def test_publish_marks_event_only_after_ack(tmp_path):
    ...
    try:
        ...
        assert subject == "guardian.pki.certificate.issued"
...
def test_failure_retains_event_and_attempts_once_per_poll(tmp_path):
    engine, factory = _database(tmp_path)
    try:
