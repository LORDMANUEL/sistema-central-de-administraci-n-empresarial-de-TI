...
class IdentityAccessVerifier:
    ...
    def _load_jwks(self, *, force: bool = False) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            try:
                ...
                raise GuardianError(503, "pki.identity_unavailable", "Identity public keys are unavailable") from exc
            if not isinstance(payload, dict) or not isinstance(payload.get("keys"), list):
                raise GuardianError(503, "pki.identity_invalid_jwks", "Identity public key response is invalid")
            ...
            for item in self._load_jwks(force=force).get("keys", []):
                if item.get("kid") == kid:
                    try:
                        ...
                        raise GuardianError(401, "pki.invalid_token", "Identity access token is invalid") from exc
        raise GuardianError(401, "pki.invalid_token", "Identity access token is invalid")
    ...
    def verify(self, token: str) -> IdentityPrincipal:
        try:
            ...
            raise GuardianError(401, "pki.token_expired", "Identity access token has expired") from exc
...
def enforce_pki_admin(
    ...
    if not decision.allowed:
        raise GuardianError(403, "pki.access_denied", "You do not have access to this tenant")
    if decision.tenant_status != "active":
        raise GuardianError(403, "pki.tenant_suspended", "Tenant is suspended")
    if decision.role != "org_admin":
        raise GuardianError(403, "pki.org_admin_required", "Organization administrator role is required")
    return decision
...
def current_principal(
    ...
    if credentials is None or credentials.scheme.lower() != "bearer":
        raise GuardianError(401, "pki.authentication_required", "Authentication is required")
