...
from .routes import AuthMode, RouteRegistry, build_route_policies
...
def create_app(
    *,
    ...
    identity_verifier: IdentityAccessVerifier | None = None,
    http_client: httpx.AsyncClient | None = None,
    audit_publisher: AuditEventPublisher | None = None,
    rate_limiter: TokenBucketLimiter | None = None,
) -> FastAPI:
    resolved = settings or Settings()
    route_registry = RouteRegistry(build_route_policies(resolved))
    ...
    resolved_audit_publisher = audit_publisher or NatsJetStreamAuditPublisher(
        resolved.nats_url,
        resolved.nats_stream,
        connect_timeout_seconds=resolved.nats_connect_timeout_seconds,
    )
    ...
    async def lifespan(app: FastAPI):
        try:
            ...
            if owned_audit_publisher:
                await app.state.audit_publisher.close()

    app = FastAPI(
        title="IT Guardian Gateway Service",
        version="0.5.0-dev.1",
        lifespan=lifespan,
    )
    ...
    async def starlette_http_error(request: Request, exc: StarletteHTTPException) -> JSONResponse:
        request_id = normalize_request_id(
            ...
            max_length=resolved.max_request_id_length,
        )
        ...
        if exc.status_code in {404, 405}:
            ...
            return JSONResponse(
                ...
                content={
                    "error": {
                        "code": "gateway.route_not_allowed",
                        "message": "Route is not exposed by the Gateway",
    ...
    def endpoint_for(route_id: str):
        async def endpoint(request: Request) -> Response:
            return await handle_gateway_request(request, route_id)

        endpoint.__name__ = f"gateway_{route_id.replace('.', '_').replace('-', '_')}"
        return endpoint

    for policy in route_registry.policies:
        if policy.auth_mode == AuthMode.INTERNAL_ONLY:
            continue
        app.add_api_route(
            policy.path_template,
            endpoint_for(policy.route_id),
            methods=[policy.method],
            name=policy.route_id,
        )
