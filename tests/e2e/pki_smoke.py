...
def verify_chain(leaf: x509.Certificate, chain: list[x509.Certificate]) -> None:
    ...
    intermediate.public_key().verify(leaf.signature, leaf.tbs_certificate_bytes, padding.PKCS1v15(), leaf.signature_hash_algorithm)
    root.public_key().verify(intermediate.signature, intermediate.tbs_certificate_bytes, padding.PKCS1v15(), intermediate.signature_hash_algorithm)
    root.public_key().verify(root.signature, root.tbs_certificate_bytes, padding.PKCS1v15(), root.signature_hash_algorithm)
...
async def fetch_event(subject: str) -> dict:
    ...
    try:
        ...
        assert messages, f"No event received for {subject}"
...
def main() -> None:
    ...
    status, _ = request(
        ...
        {"email": identity_email, "display_name": "PKI Smoke Administrator", "password": identity_password},
    )
    assert status == 201
    ...
    enrollment_private_key = Ed25519PrivateKey.from_private_bytes(decode_seed(os.environ["PKI_SMOKE_ENROLLMENT_SEED"]))
    ...
    status, listed = request("GET", f"{PKI}/api/v1/certificates?tenant_id={tenant_id}", token=admin_token)
    assert status == 200
    ...
    intermediate.public_key().verify(crl.signature, crl.tbs_certlist_bytes, padding.PKCS1v15(), crl.signature_hash_algorithm)
    ...
    print(json.dumps({"status": "ok", "certificate_id": issued["certificate_id"], "serial_hex": issued["serial_hex"], "events_verified": 2}, separators=(",", ":"), sort_keys=True))
